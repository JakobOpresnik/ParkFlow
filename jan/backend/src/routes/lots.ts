...
router.post('/', async (req, res, next) => {
  try {
    const { name, description, image_filename, image_width, image_height, sort_order } =
      req.body as {
        name?: string
        description?: string
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    if (parseInt(spotsCheck.rows[0].count) > 0) {
      res.status(409).json({
        error: 'Cannot delete a lot that still has parking spots. Remove or reassign spots first.',
      })
      return
    }
