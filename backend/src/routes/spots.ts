...
router.post('/', async (req, res, next) => {
  try {
    ...
    const spotStatus = status && validStatuses.includes(status) ? status : 'free'
  ...
  try {
    ...
    if (coordinates !== null && coordinates !== undefined) {
      ...
      if (
        ...
        res.status(400).json({ error: 'coordinates must have numeric x, y, width, height, rotation' })
        return
      }
    }
...
router.patch('/:id/owner', async (req, res, next) => {
  try {
    ...
    const before = await pool.query('SELECT owner_id FROM spots WHERE id = $1', [id])
...
router.patch('/:id/status', async (req, res, next) => {
  try {
    ...
    const before = await pool.query('SELECT status FROM spots WHERE id = $1', [id])
    if (before.rows.length === 0) {
      res.status(404).json({ error: 'Spot not found' })
      return
    }
